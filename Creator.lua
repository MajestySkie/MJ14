...
-- Tambahkan Fungsi CreateDragableImage
function Creator.CreateDragableImage(parent, assetId, size, position, onClick)
    local dragImage = Instance.new("ImageButton")
    dragImage.Size = UDim2.new(0, size.X, 0, size.Y)
    dragImage.Position = UDim2.new(position.X.Scale, position.X.Offset, position.Y.Scale, position.Y.Offset)
    dragImage.AnchorPoint = Vector2.new(0.5, 0.5)
    dragImage.Image = "rbxassetid://98803367684553" .. assetId
    dragImage.BackgroundTransparency = 1
    dragImage.Parent = parent
    ...
    dragImage.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = false
        end
    end)
